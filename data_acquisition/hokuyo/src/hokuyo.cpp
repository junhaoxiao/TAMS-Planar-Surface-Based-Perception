...
hokuyo::Laser::setToSCIP2()
{
	...
	PCL_DEBUG("Laser comm protocol changed to %s \n", buf);
}
...
hokuyo::Laser::readTime(int timeout)
{
  ...
  if (laser_time == last_time_)
  {
    if (++time_repeat_count_ > 2)
    {
      ...
      PCL_DEBUG("The timestamp has not changed for %d reads. Ignoring for now.", time_repeat_count_);
  }
  else
  {
...
int hokuyo::Laser::pollScan(hokuyo::LaserScan& scan, bool intensity, int &min_step, int &max_step, int cluster, int timeout)
{
  if (!portOpen())
    HOKUYO_EXCEPT(hokuyo::Exception, "Port not open.");
  ...
  sprintf(cmdbuf,"GD%.4d%.4d%.2d", min_step, max_step, cluster);
  ...
  // Populate configuration
  scan.config.min_angle  =  (min_step - afrt_) * (2.0*M_PI)/(ares_);
  scan.config.max_angle  =  (max_step - afrt_) * (2.0*M_PI)/(ares_);
  scan.config.ang_increment =  cluster*(2.0*M_PI)/(ares_);
  scan.config.time_increment = (60.0)/(double)(rate_ * ares_);
  scan.config.scan_time = 0.0;
  scan.config.min_range  =  dmin_ / 1000.0;
  ...
  long long inc = (long long)(min_step * scan.config.time_increment * 1000000000);
...
///////////////////////////////////////////////////////////////////////////////
int
hokuyo::Laser::requestScans(bool intensity, const int min_step, const int max_step, int cluster, int skip, int count, int timeout)
{
  if (!portOpen())
    HOKUYO_EXCEPT(hokuyo::Exception, "Port not open.");
  ...
  sprintf(cmdbuf,"M%c%.4d%.4d%.2d%.1d%.2d", intensity_char, min_step, max_step, cluster, skip, count);
...
long long int hokuyo::Laser::getHokuyoClockOffset(int reps, int timeout)
{
  ...
  for (int i = 0; i < reps; i++)
  {
    ...
    //printf("\n%lli %lli %lli", hokuyostamp, (prestamp + poststamp) / 2, offset[i]);
  }
  sendCmd("TM2",timeout);
...
hokuyo::Laser::calcLatency(bool intensity, double min_ang, double max_ang, int clustering, int skip, int num, int timeout)
{
  PCL_DEBUG("Entering calcLatency.");
  ...
  {
    PCL_INFO("Hokuyo firmware version %s detected. Using hard-coded time offset of -23 ms.",
        buggy_version.c_str());
    offset_ = -23000000;
  }
  else
