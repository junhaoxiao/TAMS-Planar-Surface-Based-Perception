...
#undef DEF_EXCEPTION
  ...
  public:
     ...
     * \param min_step   Minimal step of the scan
     * \param max_step   Maximum step of the scan
     * \param clustering Number of adjascent ranges to be clustered into a single measurement.
     * \param timeout    Timeout in milliseconds.
     *
     * \return Status code returned from hokuyo device.
     */
    int pollScan(LaserScan& scan, bool intensity, int &min_step, int &max_step, int clustering = 0, int timeout = -1);
    ...
    /** \brief Request a sequence of scans from the hokuyo
     *  This function will request a sequence of scans from the hokuyo.  If
     *  indefinite scans are requested, stop_scanning() must be called
     *  to terminate scanning.  Service_scan() must be called repeatedly
     *  to receive scans as they come in.
     ...
     * \return Status code returned from hokuyo device.
     */
    int requestScans(bool intensity, const int min_step, const int max_step, int clustering = 0, int skip = 0, int num = 0, int timeout = -1);
    ...
    /**Get the front step number afrt.*/
    int afrt();
